 ...
 * ocean · cosmos · physics · click · filters
 */
const PAGES = [
  ...
  { id: "filters", href: "./filters/index.html", label: "필터", title: "카메라 AR 필터" },
];
...
function mountPagesNav() {
  ...
  const nested = location.pathname.includes("/filters/");
  nav.innerHTML = PAGES.map(
    (p) =>
      `<a class="page-link${p.id === current ? " active" : ""}" href="${nested ? p.href.replace("./", "../") : p.href}" title="${p.title}" data-page="${p.id}">${p.label}</a>`
  ).join("");
  // 허브로 가는 홈
  const home = document.createElement("a");
  home.href = nested ? "../index.html" : "./index.html";
